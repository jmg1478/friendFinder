...
var matchYourFriend = require('../data/friends.js');

module.exports = function(app) {
  app.get('/api/friends', function (req, res) {
    res.json(matchYourFriend);
  });
  app.post('/api/friends', function (req, res) {

    var newFriend = req.body;
    for(var i = 0; i < newFriend.scores.length; i++) {
      if(newFriend.scores[i] == "1 (no)") {

        newFriend.scores[i] = 1;
      } else if(newFriend.scores[i] == "5 (yes)") {
    ...
    matchYourFriend.push(newFriend);
    res.json(matchYourFriend[bestFriendI]);
  });
};